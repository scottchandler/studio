...
angular.module('localization', []).
    factory('localize', ['$http', '$rootScope', '$window', '$filter', function ($http, $rootScope, $window, $filter) {
        // console.log('hello localize', $window.navigator.userLanguage, $window.navigator.language);

        var localize = {

            // use the $window service to get the language of the user's browser
            // language: $window.navigator.userLanguage || $window.navigator.language,
            language: Dukt_videos.language,
            ...
            initLocalizedResources:function () {
                ...
                var url = DuktVideosCms.getResourceUrl('lib/videos/angular/i18n/' + localize.language + '.js');
                // console.log('localize url', url);
                // request the resource file
                $http({ method:"GET", url:url, cache:false }).success(localize.successCallback).error(function () {
                    // the request failed set the url to the default resource file
                    var url = DuktVideosCms.getResourceUrl('lib/videos/angular/i18n/en-US.js');
